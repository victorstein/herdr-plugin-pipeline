...
```bash
herdr worktree create --cwd <repo> --branch smoke/one --base origin/main   # the base the Dispatch line names
...
reads `worktree create --cwd <repo> --base origin/<default>`; a `--depends-on` task dispatched after
its dependency merged on GitHub is cut from a commit containing that merge even though nobody pulled
local `main` (`git -C <worktree> merge-base --is-ancestor <merge sha> HEAD` exits 0).

...
| The artifact path still overruns `TICK_MS` | Task 15 collapsed the actor settles into one shared concurrent window, but `isSettled` sleeps `FILE_SETTLE_MS` (750ms) inline — once in `evaluateRun` and again per task inside `verdictFor` — all serial. A tick advancing several artifact rows therefore still exceeds `TICK_MS = 1000`. Same shape as the actor problem and the same fix (one shared window, memoised by path), but it was outside Task 15's scope. Measure on the live run before deciding whether it matters: the supervisor skips rather than queues ticks, so the cost is latency, not backlog. |
...
| Answer delivery burns budget on non-retryable failures | `deliverPendingAnswers` increments `delivery_attempts` on any failure, where `main.ts`'s own loop routes through `deliver.ts`'s `shouldRetry` and classifies. So a terminal `pane_not_found` consumes five ticks before settling instead of stopping at once. It fails safe — the task stays blocked with the answer preserved — but the two paths now classify failures differently. Route both through `shouldRetry` if this shows up live. |
...
| The phase table is less authoritative than it looks | `onClear`/`onBlocker`/`artifact`/`stallable`/`holdsFiles`/`probeTarget`/`stallWhen` are genuinely read. `counter`, `resumePrompt`, `resumeActor` and `returnsTo` are not — the implementations hardcode their equivalents, and only `test/table.test.ts` pins them. `prompt` is read for the five design rows only; every other row duplicates a literal at its call site. Someone editing a row expecting behaviour to follow would be wrong about most of them. Either consult the fields or drop them; do not leave the table looking like a mechanism when it is partly documentation. |
| `Task.artifacts.verdicts` and `RunArtifacts.verdicts` are read but never written | Both maps are consulted in `deliver.ts` and seeded only by tests, so in production they are permanently `{}` and the `docs/superpowers/reviews/…` fallback is always taken. Either an unfinished "the agent names its own verdict path" hook, or dead weight. Decide which. |
| The `worktree.removed` event is enqueued and silently dropped | `EventKind` includes it, the manifest registers the hook, `src/hooks/worktree-removed.ts` enqueues it — and `applyEvents` has no branch for it, so it falls through every `kind` check. `teardown.ts`'s comment implies it was meant as idempotency protection. Wire it or remove the hook and its manifest entry. |
| `test/prompts.test.ts`'s prompt list is hand-maintained | Its "no orphan prompt files" assertion cannot catch an orphan, which is why `prompts/digest.md` has survived unrendered since the first commit — its content is duplicated in TypeScript by `buildDigest`. Derive the list from the prompt names reachable in `src/` and the test becomes load-bearing. |
| Task 28's test does not drive the real tick | It exercises `isCurrentSchemaRun` and mirrors the filter, but `main()` is an unexported loop over live env vars, so removing `.filter(isCurrentSchemaRun)` from `main.ts` would leave the test passing. Extracting the tick body would make this and several other supervisor behaviours testable. |

...
Revised in place after `docs/superpowers/reviews/issue-26-plan-review-0.md` cleared it with 3 MAJORs
and 5 MINORs; all eight are applied. The MAJORs were gaps rather than errors: the spec's live
runbook mapped to no step (now step 10), the run-level `cmdRewind` reservation had no test (now the
third test in step 7), and nothing pinned `verdict_seq` against being cleared by `cmdRewind` — the
producer-row test asserted `undefined` on a fixture that never set it, which an implementation that
*clears* the counter would also satisfy.
...
**Steps 1-8 are each one TDD cycle** — write the test, run it and *watch it fail*, write the minimum
code, run it again, commit. No step ends with a red tree. Step 9 opens the PR; step 10 is the live
runbook, which cannot run until a release carrying this change is installed. Run the whole suite
before each commit:
...
there is one less guard to keep in sync with the reservation guards in steps 5, 6 and 7.

That equivalence has a second half worth stating, because "behaviour is identical" is exactly the
kind of claim this spec's reviews kept catching. Spec C3's "today's derivation" for a non-verdict row
is `task.artifacts.verdicts[key] ?? join(...)` (`src/supervisor/deliver.ts:101-103`) — a map lookup
first. The fallback below has **no map lookup at all**, so for a non-verdict row carrying a recorded
key the two differ. They are identical only given spec **A1**: nothing has ever written
`artifacts.verdicts`, the only writes being the `{}` initialisers at `src/cli.ts:229` and
`src/lib/ledger.ts:35`, so no record on disk has such an entry.
...
**Replace** the whole import block at the top of `test/verdict-path.test.ts` with this — step 1's
file has no `node:fs`/`node:path` imports to extend, and re-adding `cleanupFixtures` on a second line
is a duplicate binding and a `SyntaxError` at module load, which is not a useful red:
...
(`reserveVerdict` is unused until **step 6**, which is its first consumer; it is imported now so the
import line is touched once. `tsconfig.json` sets `strict` and `noUncheckedIndexedAccess` but not
`noUnusedLocals`, so the unused name is not an error.)
...
test('a rewind to a producer row reserves nothing and does not reset verdict_seq', async () => {
  ...
  // the one property C1 rests on — that the key never regresses.
...
The third test is what covers item 5 below — without it the run branch of `cmdRewind` ships
untested, because the other two both pass `taskId: 't1'`.
...
**Files:** `test/prompts.test.ts`, `prompts/escalate.md`, `test/integration/smoke.md`.

This step has a test like every other. `test/prompts.test.ts:124-125` and `:134-137` already
establish the pattern for pinning a corrected prompt sentence with `not.toContain`, twice, for
exactly this class of fix. Append to `test/prompts.test.ts`:
...
Run `bun test test/prompts.test.ts`. It fails on both assertions.
...
`test/integration/smoke.md:547` carries the same false sentence about the same command — *"which
resets that phase's pass count"* — and `:544` lists what a rewind clears without the new reserved
path. Correct both in this step, since leaving one of two copies right is how the next reader
concludes the corrected one is the mistake. In the `:547` row, replace *"which resets that phase's
pass count"* with:

    which clears every pass counter on that record and, for a review phase, prints the fresh verdict
    path it reserved

and append to the `:544` row, after the `escalated_from` clause:

    Rewinding *onto* a review phase also reserves a fresh verdict path and prints it; write the next
    review there.
...
`src/supervisor/tasks.ts`, `src/cli.ts`, `prompts/escalate.md`, `test/integration/smoke.md`,
`test/prompts.test.ts`, `test/verdict-path.test.ts`, `test/deliver.test.ts`, `test/tasks.test.ts`,
`test/cli-commands.test.ts`. **Nothing else.**
...
## Step 10 — live verification, after the release lands

**Not skippable, and not runnable before the merge.** `.claude/agents/plugin-dev.md` requires it
because this change touches delivery — what a prompt tells an agent to write — and that guide records
that this repo's dependency-injected unit suite has passed clean over real startup/gating defects
twice. `bun test` being green is not evidence for this step.

It cannot run earlier: the installed plugin is a GitHub install pinned to a tag
(`herdr plugin list` → `…@be181757…`), so the supervisor driving any live run is *not* this checkout.
Do not link `src/cli.ts` onto PATH or run it against live state to shortcut this — the CLI and the
supervisor share one ledger, and a drifted schema either throws inside the supervisor pane or makes
the run silently invisible.

Once a release carrying this change is installed, on a real herdr session:

1. `hpipe rewind <run> spec --task <t>` on a task with `…-spec-review-0.md` and `-1.md` committed.
   The re-rendered prompt names `…-2.md`, the ledger shows key `spec-review-2`, and `-0`/`-1` are
   untouched in `git status`.
2. `hpipe rewind <run> spec-review --task <t>` onto the row, last verdict `CLEAR`. The command's own
   output names the reserved path and the ledger agrees. **The case no unit test proves end to end**,
   because it needs a real `cmdRewind` process writing a ledger the supervisor then reads.
3. Immediately after step 2, confirm the row does **not** advance on its own, that the first stall
   probe at `TASK_STALL_MINUTES` (45) names the reserved path, and that the earlier verdicts are
   untouched. This is spec A6's accepted cost, and the CLEAR spec review promoted it to the primary
   check — observe it deliberately so a later run does not report it as a stall bug.
4. Raise a decision on a task in `spec-review`, answer it with `hpipe answer`, and confirm
   `verdict_seq` and the resolved path **do not move**. This is the failure the orchestrator's ruling
   exists to prevent, and the reason the counter is not bumped on `enterTaskPhase`.
5. `hpipe status` still prints `pass N` from the escalation budget, not the file ordinal (A6, A7).

Any difference between this runbook and what is observed is a **finding**, not a test to make pass —
file it rather than adjusting the code to match the runbook.

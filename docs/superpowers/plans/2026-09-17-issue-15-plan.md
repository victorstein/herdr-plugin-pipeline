...
**This is the only step that creates a new file, so it is the only one that cannot use `-am`:**
`git commit -am` stages modifications to *tracked* files and silently ignores untracked ones. No
workflow in `.github/workflows/` runs `bun test` (there is only `pr-title-lint.yml` and
`release-please.yml`), so nothing downstream would catch the omission — the branch would simply push
without the prompt and every escalation would throw at delivery.

```
git add prompts/stall-escalate.md test/prompts.test.ts
...
Two hunks, leaving the parts between them alone. **Anchored on symbols, not line numbers** — step 6
inserted an import into this same file, so every line number from the original source has shifted:

- **delete** from `export interface StallCandidate {` through the closing brace of
  `export function stallKey`
- **keep** the `probePaneFor` doc comment and function
- **delete** from `export function stallCandidates(` through the closing brace of
  `export function taskStallCandidates` — this span also contains `interface TaskStallCandidate`
  and `taskStallKey`
- **keep** `sendProbes` (step 10 removes it)
- **keep** everything you appended in steps 3–7

Insert, in place of the second deleted span:
...
**Test** — append to `test/stall.test.ts`, adding `applyStalls` and `type StallDeps` to the
`../src/supervisor/stall` import, and `saveRun`, `listRuns` from `../src/lib/ledger`, plus
`import { mkdtempSync } from 'node:fs'`, `import { tmpdir } from 'node:os'` and
`import { join } from 'node:path'` (the pattern `test/config.test.ts:2-4` uses). `AgentStatus` is
**not** needed — `TestDeps` annotates the literal, which contextually types `agentStatus`:
...
test('a bump survives the ledger round trip that every tick performs', async () => {
  ...
  // advances across ticks and P4 is back. A fake cannot show this: the object
  // re-read above is the same one the bump mutated.
...
**Two things are deliberately not unit-tested**, both for the same reason — the real `escalate` is an
anonymous literal built inside `main()` (step 10's wiring below), so nothing exported can reach
them. Both are covered by live checks in step 16, and both are stated here rather than left to be
discovered as gaps:

- **A22** (spec:609) — *"an escalation whose send fails still leaves the record in `escalated`"*.
  Live check 4. The ordering is enforced by reading the wiring: the `enterTaskPhase`/`enterRunPhase`
  and `saveRun` calls precede every `return` and the send.
- **A27's reason string** (spec:606) — *"the reason string reports probes only"*. The
  `${c.probes} stall probes unanswered` literal is inside that same callback. Live check 8.
...
```
bun test              # expect: 387 pass, 0 fail
bun run typecheck     # expect: silent
git status --porcelain  # MUST be empty — catches a file created but never staged
git log --oneline -16
git push
```

`git status --porcelain` is not ceremony: step 8 creates the only new file in this plan, and no
workflow runs the test suite, so an unstaged prompt would reach `main` undetected.
...
| **MINOR 4** — four spec-named tests unmapped | `verdict` (task and run) and the fallback added to step 6; the rendered-`{{` assertion added to step 11; **A22** stated as a deliberate unit-test exception in step 10 and moved to live check 4. **The fourth item, the persistence round trip, was missed here and is fixed below (pass-1 MAJOR 2).** |

## What changed after the second plan review

`docs/superpowers/reviews/issue-15-plan-review-1.md` — **CLEAR**, 0 BLOCKERs / 2 MAJORs / 3 MINORs,
reached by executing all sixteen steps against a copy of the worktree (final: 387 pass / 0 fail).
All five fixed inline.

| Finding | Fix |
| --- | --- |
| **MAJOR 1** — step 8 committed a newly created file with `git commit -am`, which cannot stage an untracked file; with no workflow running `bun test`, the branch would push without `prompts/stall-escalate.md` | Step 8 now uses explicit `git add` and a `git status --porcelain` check, with the reasoning stated; step 16 adds the same check before `git push`. |
| **MAJOR 2** — the spec's persistence round trip (spec:594-595) was unmapped while the table claimed pass-0's MINOR 4 fixed; only 3 of its 4 items were | Added to step 10 as a real `saveRun`/`listRuns` round trip, with a comment saying why the fake-`persist` test above it is insufficient. |
| **MINOR 1** — step 9c's line ranges had drifted, because step 6 inserts an import into the same file | Re-anchored on symbol boundaries, which do not move when an earlier step edits the file. |
| **MINOR 2** — two clauses of the spec's A27 test were unmapped and unacknowledged | The anchor assertion added to step 10's deferral test; the reason string joined **A22** in an explicit "deliberately not unit-tested" note naming its live check. |
| **MINOR 3** — step 10 instructed adding a `type AgentStatus` import its own tests never use | Removed, with a note that `TestDeps`'s annotation is what types `agentStatus`. |

...
| `herdr agent prompt` does **not** clear the input box: text already typed there is prepended to the prompt and submitted with it, and the call reports success. `--wait --until working --until blocked` returns in ~0.5s on a Claude that takes the prompt up | live, isolated `--session`, Claude Code 2.1: `STRAY` + prompt arrived as one submitted message | Delivery is confirmed by take-up, never by the write (#18) |
| `send-keys` takes logical names: `C-u` is `invalid_key`. On an idle Claude, `ctrl+u` deletes one line of the box, `ctrl+c` empties it whole, `esc esc` on an empty box opens the rewind menu; `ctrl+c` on an empty box arms exit for under 2s | live, isolated `--session`, Claude Code 2.1 | A stalled send is cleared with one `ctrl+c` only on a worker pane, only when the agent reads idle (re-read just before the press) AND the box holds nothing but paste placeholders or pieces of that send, and never twice to one pane within 10s (`ClearGuard`); any other text is reported as stuck input, never cleared |
| Agent commands on a missing target return `agent_not_found`, pane commands `pane_not_found` — whether the pane is closed or only its agent has exited — as a stderr envelope with exit 1 | live, bogus ids | Both are retryable: a pane can come back under a restore |
